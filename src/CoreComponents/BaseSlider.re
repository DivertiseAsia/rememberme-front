...
let isSlideInRange =
    (slideNumber: int, currentSlideNumber: int, maxSlides: int, offset: int) =>
...
let renderSlide = (slide: slide, offset: int) => {
  ...
  <div className key={"slide-" ++ string_of_int(slide.slideNumber)}>
    {slide.content}
  </div>;
};
[@react.component]
let make =
  ...
  <div
    className={
      "slider " ++ Js.Option.getWithDefault("slider-default", className)
    }>
