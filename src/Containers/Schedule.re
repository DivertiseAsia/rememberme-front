...
let make =
  ...
  let (state, dispatch) =
    React.useReducer(
      (state, action) => {
        switch (action) {
        | ChangeScheduleMenu(scheduleMenu) => {...state, scheduleMenu}
        | ChangeContentForm(contentForm) => {...state, contentForm}
        | TogglePopover(showPopover) => {...state, showPopover}
        }
      },
      {
        loadState: Idle,
  ...
  <div
    ...
    <div className="row schedule-row-menu">
      ...
      <div className="col-6 text-right">
        <span
          className="cursor-pointer"
          onClick={_ => dispatch(TogglePopover(!state.showPopover))}>
          {string(
             switch (profile) {
             | Some(p) => p.firstName
             | None => "-"
        ...
        {state.showPopover
           ? <Popover onClose={_ => dispatch(TogglePopover(false))} /> : null}
      </div>
    </div>
    {switch (state.contentForm) {
     | Idle =>
             ...
             <div className="row">
               {menus
                |> List.map((menu: scheduleMenu) =>
                     <div
                       key={
                         "menu-" ++ (menu |> RememberMeUtils.scheduleMenuToStr)
                       }
               ...
               {let today =
                ...
                | (All, allBirthDay, allHoliday, allLeave, allEvent)
                  ...
                  let birthdaySchedules =
                    ...
                    |> List.map(birthDay =>
                         birthDay |> RememberMeUtils.mapBirthDayToSchedule
                       );
                  ...
                  |> List.mapi((i, schedule: schedule) =>
                       ...
                       | date when date > lastEndDate[0] =>
                         let datetime =
                           ...
                           | Birthday =>
                             schedule.date
                             |> RememberMeUtils.getDatetimeStr(
                         ...
                         lastEndDate[0] =
                           (schedules |> List.rev |> List.hd).date;
                         <div
                           key={
                             "scheduler-all-"
                             ++ schedule.title
