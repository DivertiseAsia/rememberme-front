    ...
    <div className="col-3 col-md-3 p-1">
      <div className="dropdown show dropdown-request-leave">
        ...
        <div
          ...
          style={ReactDOMRe.Style.make(~maxHeight="200px", ~overflowY="auto", ())}>
          {Array.make(31, null)
           |> Array.mapi((idx, _) => {
                let today =
                  Js.Date.make()
                  ...
                  |> Js.Date.valueOf;
                let datetime =
                  Js.Date.makeWithYMD(~year=targetYear, ~month=targetMonth, ~date=idx + 1 |> float_of_int, ());
                let isLeaveDay =
                  switch (schedules |> List.find(schedule => schedule.date === (datetime |> Js.Date.valueOf))) {
                  | _leaveDay => true
          ...
          {months
           |> List.mapi((i, month) =>
                <a
                  ...
                  onClick={_ => {
                    let datetime =
                      Js.Date.makeWithYMD(~year=targetYear, ~month=i |> float_of_int, ~date=targetDate, ());
...
let make = (~schedules, ~onRefresh, _children) => {
  ...
  reducer: (action, state) => {
    ...
    | ChangeStartDate(startDate) =>
      Update({...state, startDate, endDate: startDate > state.endDate ? startDate : state.endDate})
  ...
  render: ({state, send}) =>
    <div className="container mt-4 request-form-container">
      ...
      <div
        ...
        style={ReactDOMRe.Style.make(~backgroundColor="white", ~borderTop="2px solid #FFA227", ())}>
        {requestMenus
        ...
        <div className="col-12">
          ...
          {switch (state.formType) {
           | Vacation =>
             <div className="row mt-3"> <div className="col"> <img src="/images/coconut.svg" /> </div> </div>
           | _ => null
           }}
        </div>
      </div>
