...
let make = (~requestLeaves, ~onRefresh) => {
  let (state, dispatch) = React.useReducer((state, action) =>
    switch action {
    | ChangeFormMenu(formMenu) => {...state, formMenu: formMenu}
  ...
  <div className="container request-form-container">
      ...
      <div className="col-5 col-md-6 col-lg-5 col-xl-3 pl-0">
        <button
          ...
          className={"btn btn-rounded " ++ (
            state.formMenu === History ? "btn-main-color active-menu" : "btn-outline"
          )}
    ...
    <div
      ...
      style={ReactDOM.Style.make(~backgroundColor="white", ~borderTop="2px solid #52C4BB", ())}>
      {requestLeaves
