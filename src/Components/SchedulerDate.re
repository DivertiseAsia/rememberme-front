...
let make = (~isToday=false, ~scheduleList=[], ~datetime="", ~schedules: list(RememberMeType.schedule), _children) => {
  ...component,
  render: _self => {
    <div className="container mt-3">
      <div className="row">
        <div className="col">
          ...
          <hr
            className="mt-0"
            style={ReactDOMRe.Style.make(~borderTop=isToday ? "1px solid #2D506D" : "1px solid rgba(0,0,0,.1)", ())}
          />
        </div>
      </div>
      {schedules
       |> List.mapi((i, schedule: RememberMeType.schedule) => {
